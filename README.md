...
```bash
bash <(wget -qO- https://raw.githubusercontent.com/leejayhsu/touchkio/main/install.sh)
```

Make sure that you run this with your **standard user** and not with root (sudo).
If you are paranoid, or smart, or both, have a look into the [install.sh](https://github.com/leejayhsu/touchkio/blob/main/install.sh) script before executing external code on your machine.
...
The [install.sh](https://github.com/leejayhsu/touchkio/blob/main/install.sh) script can also be run to update an existing installation to the **latest version**.
The setup procedure can be skipped to use the existing default arguments from the configuration file:

```bash
bash <(wget -qO- https://raw.githubusercontent.com/leejayhsu/touchkio/main/install.sh) update
```
...
```json
{
  "web_url": ["https://demo.home-assistant.io", "https://demo.immichkiosk.app"]
}
```
